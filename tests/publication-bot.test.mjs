...
test('selects the oldest trusted publication PR without relying on recent Slack roots', () => {
  ...
  const reservedCharacters = nextOpenPublicationPr([{
    ...trusted(54, '10.1234/a?b#c'),
    body: '## Publication\n\n- **DOI:** https://doi.org/10.1234/a%3Fb%23c'
  }], repository);
...
test('inserts an approved candidate without changing existing entries', () => {
  ...
  assert.match(updated, /F\('73', 'New paper', 'Chung, Yongchul G\.', 'doi', 'New Journal'/);
  ...
  assert.throws(
    () => addCandidateToFeed(feed, { ...candidate, doi: '' }),
    /DOI is required/
  );
});
